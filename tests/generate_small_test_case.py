...
def parse_args() -> argparse.Namespace:
    ...
    grid.add_argument("--resolution", type=float, default=0.05,
                      help="Grid resolution in degrees")
    grid.add_argument("--xstart", type=float, default=278.0,
                      help="Minimum longitude [0, 360]")
    grid.add_argument("--lenx", type=float, default=3.0,
                      help="Longitude extent in degrees")
    grid.add_argument("--ystart", type=float, default=7.0,
                      help="Minimum latitude [-90, 90]")
    grid.add_argument("--leny", type=float, default=3.0,
                      help="Latitude extent in degrees")
    grid.add_argument("--name", type=str, default="panama1",
                      help="Grid/domain name")

    # --- Topography ---
    topo = parser.add_argument_group("Topography")
    topo.add_argument("--min-depth", type=float, default=9.5,
                      help="Minimum ocean depth in meters")
    topo.add_argument("--bathymetry-path", type=Path, default=None,
                      help="Path to existing bathymetry file (GEBCO .nc). "
                           "If omitted, GEBCO data will be downloaded to ./GEBCO.nc")
    topo.add_argument("--lon-coord", type=str, default="lon",
                      help="Longitude coordinate name in the bathymetry dataset")
    topo.add_argument("--lat-coord", type=str, default="lat",
                      help="Latitude coordinate name in the bathymetry dataset")
    topo.add_argument("--elev-coord", type=str, default="elevation",
                      help="Vertical/elevation coordinate name in the bathymetry dataset")

    # --- Vertical grid ---
    vgrid = parser.add_argument_group("Vertical Grid")
    vgrid.add_argument("--nk", type=int, default=75,
                       help="Number of vertical levels")
    vgrid.add_argument("--vgrid-ratio", type=float, default=20.0,
                       help="Target ratio of top-to-bottom layer thicknesses")

    # --- CESM case ---
    case = parser.add_argument_group("CESM Case")
    case.add_argument("--casename", type=str, default="panama-crocontainer",
                      help="CESM experiment/case name")
    case.add_argument("--cesmroot", type=Path, default=Path("/home/manishrv/CROCESM"),
                      help="Path to CESM source root")
    case.add_argument("--inputdir", type=Path, default=None,
                      help="Directory for CESM input files "
                           "(default: /home/manishrv/croc_input/<casename>)")
    case.add_argument("--caseroot", type=Path, default=None,
                      help="CESM case directory "
                           "(default: /home/manishrv/croc_cases/<casename>)")
    case.add_argument("--project", type=str, default="CESM0030",
                      help="HPC project/account code")
    case.add_argument("--machine", type=str, default="ubuntu-latest",
                      help="CIME machine name")
    case.add_argument("--compset", type=str,
                      default="1850_DATM%NYF_SLND_SICE_MOM6%REGIONAL_SROF_SGLC_SWAV_SESP",
                      help="CESM compset alias or longname")
    case.add_argument("--atm_grid_name", type=str,
                      default="T62",
                      help="CESM atm grid name (T62 for NYF)")
    case.add_argument("--no-override", dest="override", action="store_false",
                      help="Do not overwrite an existing case directory")

    # --- Forcings ---
    forcings = parser.add_argument_group("Forcings")
    forcings.add_argument("--date-start", type=str, default="2020-01-01 00:00:00",
                          help="Forcing start date (YYYY-MM-DD HH:MM:SS)")
    forcings.add_argument("--date-end", type=str, default="2020-01-05 00:00:00",
                          help="Forcing end date (YYYY-MM-DD HH:MM:SS)")
    forcings.add_argument("--forcing-fn", type=str,
...
def get_bathymetry(args: argparse.Namespace) -> Path:
    ...
    GEBCO.get_gebco_data_with_python("", bathy)
