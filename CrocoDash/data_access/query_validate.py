...
TABLES_DIR = Path(__file__).parent/"tables"  # Adjust this to wherever your tables are stored
...
def list_functions(product_name):
    ...
    return functions_df[functions_df["Product_Name"] == product_name]["Function_Name"].tolist()
...
def function_exists(product_name, function_name):
    ...
    return ((functions_df["Product_Name"] == product_name) & 
            (functions_df["Function_Name"] == function_name)).any()
...
def verify_data_sufficiency(collected_products: list):
    ...
    collected_categories = set(products_df[products_df["Product_Name"].isin(collected_products)]["Data_Category"])
