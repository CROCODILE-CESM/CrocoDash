...
def test_configure_yaml_for_forked_case_args(fake_fcb_empty_case, tmp_path):
    ...
    (fcb.bundle_location / "ocn").mkdir(parents=True)
    ...
    bundle_yaml = {
        ...
        "grid": {"supergrid_path": "/old/ocn/ocean_hgrid.nc"},
        "topo": {
            "source": {
                "type": "from_file",
                "topo_file_path": "/old/ocn/ocean_topog.nc",
            }
        },
        "vgrid": {"type": "from_file", "filename": "/old/ocn/ocean_vgrid.nc"},
    }
    ...
    # Grid/topo/vgrid paths are redirected to bundle ocn
