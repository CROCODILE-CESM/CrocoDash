...
def test_case_integration_config(get_CrocoDash_case):
    ...
    assert set(config["paths"].keys()) == {
        "raw_dataset_path", "hgrid_path", "regridded_dataset_path", "merged_dataset_path"
    }
    assert all(isinstance(val, str) for val in config["paths"].values())

    
    # Raw file regex
    assert isinstance(config["raw_file_regex"], dict)
    assert set(config["raw_file_regex"].keys()) == {
        "raw_dataset_pattern", "regridded_dataset_pattern"
    }
    assert all(isinstance(val, str) for val in config["raw_file_regex"].values())
    ...
    assert set(config["forcing"].keys()) == {"product_name", "function_name", "varnames"}
    ...
    assert config["paths"]["raw_dataset_path"] == str(large_data_workflow_path/"raw_data")
    assert config["paths"]["regridded_dataset_path"] ==str(large_data_workflow_path/"regridded_data")
    assert config["paths"]["merged_dataset_path"] == str(case.inputdir)
    
    # Raw file regex
    assert config["raw_file_regex"]["raw_dataset_pattern"] == "(north|east|south|west)_unprocessed\\.(\\d{8})_(\\d{8})\\.nc"
    assert config["raw_file_regex"]["regridded_dataset_pattern"] == "forcing_obc_segment_(\\d{3})_(\\d{8})_(\\d{8})\\.nc"
