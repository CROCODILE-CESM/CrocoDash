...
Three products -- REFERENCE_OCEAN (MOM6), REFERENCE_ICE (CICE), REFERENCE_WAVES
(WW3) -- that generate plausible-looking forcing data purely in memory via
numpy, with no network access, credentials, or campaign-storage dependency.
Each access method is a pure function of its own (dates, lat/lon bbox)
arguments: same inputs always produce the same output, so these are safe to
use in tests (real assertions, not just "did it not crash") and in demo
notebooks that should look identical on every run.

These are not meant to be physically accurate -- just close enough in shape
(a warm-at-equator thermocline, an ice edge that tapers with latitude, a
JONSWAP-shaped wave spectrum) that the real MOM6/CICE/WW3 regrid pipelines
have something sensible to chew on, fast.
"""
...
class REFERENCE_ICE(CICEForcingProduct):
    ...
    def get_reference_ice_data(
        ...
        # gives tlon/tlat/ulon/ulat a "time" dim, which
        # extract_forcings/cice.py's regrid step expects.
